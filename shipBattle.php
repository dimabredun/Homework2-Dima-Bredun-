...
$start = new SessionManager();
$start->start();
$finalStat = new StatCenter();
$finalStat->addFightStats($battleResult);
?>
...
<div class="container">
    ...
    <div class="result-box center-block">
        ...
        <?php if($static = $finalStat->getFightStats()): ?>
         <div class="statistic">
            <h3 class="text-center audiowide">Fight statistics</h3>
            <h2 style="text-align: center">All battles: <?= $finalStat->countBattles() ?></h2>
            <table>
                <tr>
                    <th>Battle #</th>
                    <th>Winner</th>
                ...
                <tr>
                    ...
                    <td><?= $statItem->getBattleShip()->getName() ?>
                        (qty:<?= $statItemData[\BattleResult::WINNER_KEY][\BattleResult::QTY_KEY] ?>,
                        health: <?= $statItemData[\BattleResult::WINNER_KEY][\BattleResult::START_HEALTH_KEY] ?>,
                        remain health: <?= $statItemData[\BattleResult::WINNER_KEY][\BattleResult::REMAIN_HEALTH_KEY] ?>)
                    </td>
                    <td><?= $statItem->getBattleShip(false)->getName() ?>
                        (qty:<?= $statItemData[\BattleResult::LOOSER_KEY][\BattleResult::QTY_KEY] ?>,
                        health: <?= $statItemData[\BattleResult::LOOSER_KEY][\BattleResult::START_HEALTH_KEY] ?>,
                        remain health: <?= $statItemData[\BattleResult::LOOSER_KEY][\BattleResult::REMAIN_HEALTH_KEY] ?>)
                    </td>
